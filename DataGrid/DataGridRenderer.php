...
class DataGridRenderer extends Nette\Object implements IDataGridRenderer
{
	...
	public function renderBody()
	{
		...
		// headers
		$header = Nette\Utils\Html::el($container->getName() == 'table' ? 'thead' : NULL);
		...
		// footer
		$footer = Nette\Utils\Html::el($container->getName() == 'table' ? 'tfoot' : NULL);
		$footer->add($this->generateFooterRow());

		// body
		$body = Nette\Utils\Html::el($container->getName() == 'table' ? 'tbody' : NULL);
		...
		foreach ($this->dataGrid->getColumns() as $column) {
			...
			if ($column->isOrderable()) {
				...
				if (count($list) > 1 && isset($list[$column->getName()])) {
					$text .= Nette\Utils\Html::el('span')->setHtml($list[$column->getName()][1]);
				}

				$up = clone $down = Nette\Utils\Html::el('a')->addClass(DataGridColumn::$ajaxClass);
				$up->addClass($a ? 'active' : '')->href($column->getOrderLink('a'))
					->add(Nette\Utils\Html::el('span')->class('up'));
				$down->addClass($d ? 'active' : '')->href($column->getOrderLink('d'))
					->add(Nette\Utils\Html::el('span')->class('down'));
