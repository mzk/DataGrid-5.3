...
class DataGrid extends Nette\Application\UI\Control
{
	 ...
	 *
	 * @param  mixed  callback(s) to handler(s) which is called after data grid form operation is Submitted.
	 * @return void
	 */
	public function setOnOperationSubmit($callback)
	{
		if (!is_array($this->onOperationSubmit)) {
			$this->onOperationSubmit = [];
		}
	...
	public function getRows()
	{
		if (!$this->dataSource instanceof DataSource) {
			throw new Nette\InvalidStateException("Data source has not been set or has invalid data type. You must set data source before you want get rows.");
		}
	...
	{
		return ['page', 'order', 'filters', 'itemsPerPage'];
	}
	...
	{
		if ($value < 0) {
			throw new Nette\InvalidArgumentException("Parametr must be non-negative number, '$value' given.");
		}
	...
	public function formSubmitHandler(Nette\Application\UI\Form $form)
	{
		...
		if ($form->isSubmitted()) {
			...
			if ($form['filterSubmit']->isSubmittedBy()) {
				...
				if (!is_array($this->onOperationSubmit)) {
					throw new Nette\InvalidStateException('No user defined handler for operations; assign valid callback to operations handler into DataGrid::$operationsHandler variable.');
				}
			} else {
	...
	protected function applyPaging()
	{
		...
		if ($this->wasRendered && $this->paginator->itemCount < 1 && !empty($this->filters)) {
			...
			$this->getTemplate()->flashes[] = (object) [
	...
	public function render()
	{
		...
		$s = call_user_func_array([$this->getRenderer(), 'render'], $args);
	 ...
	 */
	protected function createTemplate($lass = NULL)
	{
	...
	protected function regenerateFormControls()
	{
		...
		if ($this->hasOperations()) {
			...
			if (!empty($values['checker'])) {
				$form->setDefaults(['checker' => $values['checker']]);
			}
		}
	...
	{
		if (!$this->hasColumns('ActionColumn')) {
			throw new Nette\InvalidStateException('No ActionColumn defined. Use DataGrid::addActionColumn before you add actions.');
		}
	...
	public function translate($s)
	{
		...
		return $this->translator === NULL ? $s : call_user_func_array([$this->getTranslator(), 'translate'], $args);
	}
	...
	{
		$s = call_user_func_array([$this->getRenderer(), 'render'], [$this]);
